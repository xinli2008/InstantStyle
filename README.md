# Update:
作者发现，通过简单的特征空间操作（如特征相减）和仅在特定风格相关的注意力层中注入风格信息，可以有效实现参考图风格注入。主要包括两个部分：
1. 特征空间中的风格和内容解耦：内容特征的显式去除，由于内容（content）通常可以通过文本描述，而风格难以用文本精确描述，作者提出使用CLIP模型的文本编码器提取内容文本的特征，然后从参考图像的CLIP特征中减去内容特征，从而显式地去除内容信息，保留风格信息。
2. 风格相关的注意力层：作者通过实验发现，在扩散模型中，某些特定的注意力层（如 up_blocks.0.attentions.1 和 down_blocks.2.attentions.1）主要负责风格信息的处理。具体来说，up_blocks.0.attentions.1 负责风格（颜色、材质、氛围等），而 down_blocks.2.attentions.1 负责空间布局（结构、构图等）。作者提出仅在风格相关的注意力层中注入风格信息，而不是在所有层中都注入。这样可以有效防止内容泄漏，同时保持风格强度。
